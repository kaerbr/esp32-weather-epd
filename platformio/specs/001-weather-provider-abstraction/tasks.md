...
- [x] T001 Create `include/providers/` and `src/providers/` directories for provider implementations
...
- [x] T002 [P] Create `include/wmo_codes.h` defining the `wmo_code_t` enum with all WMO 4677 grouped categories (WMO_CLEAR, WMO_HAZE, WMO_DUST, WMO_MIST, WMO_FOG, WMO_LIGHTNING, WMO_SQUALL, WMO_TORNADO, WMO_DRIZZLE, WMO_FREEZING_DRIZZLE, WMO_RAIN, WMO_FREEZING_RAIN, WMO_RAIN_SNOW, WMO_SNOW, WMO_SLEET, WMO_HAIL, WMO_THUNDERSTORM, WMO_THUNDERSTORM_HAIL, WMO_SMOKE, WMO_VOLCANIC_ASH, WMO_CLOUDY_FEW, WMO_CLOUDY_SCATTERED, WMO_CLOUDY_BROKEN, WMO_OVERCAST, WMO_UNKNOWN) plus `isDaytime(int64_t dt, int64_t sunrise, int64_t sunset)` declaration per data-model.md and research.md R2/R4
- [x] T003 [P] Create `include/weather_data.h` defining all universal structs: `weather_condition_t`, `weather_current_t`, `weather_hourly_t`, `weather_temp_t`, `weather_feels_like_t`, `weather_daily_t`, `weather_alert_t`, `air_quality_t`, and root `weather_data_t` with constants (MAX_HOURLY=48, MAX_DAILY=8, MAX_ALERTS=8, MAX_AQ_HOURS=24) per data-model.md. All string fields MUST use fixed-size `char[]` arrays. Include `initWeatherData(weather_data_t &data)` declaration. Include sentinel check helpers: `isSentinelFloat(float)`, `isSentinelInt(int)`, `isSentinelTimestamp(int64_t)`.
- [x] T004 [P] Create `include/weather_provider.h` defining the `WeatherProvider` abstract base class with pure virtual methods `getName()` and `fetchData()` per contracts/weather_provider_interface.md. Use `#ifdef USE_HTTP` to conditionally declare the WiFiClient vs WiFiClientSecure parameter variant.
- [x] T005 Create `src/wmo_codes.cpp` implementing `isDaytime()` function (compares dt against sunrise/sunset timestamps) per research.md R4
- [x] T006 Create `src/weather_data.cpp` implementing `initWeatherData()` that sets all fields to sentinel values (NAN for float, INT32_MIN for int, 0 for int64_t, WMO_UNKNOWN for wmo_code_t, '\0' for char arrays) per data-model.md sentinel table
...
- [x] T007 [P] [US1] Create `include/providers/owm_provider.h` declaring `OpenWeatherMapProvider` class inheriting from `WeatherProvider` with `getName()` and `fetchData()` method signatures
...
- [x] T020 [US3] Update `drawStatusBar()` signature in `include/renderer.h` to accept an additional `const char *providerName` parameter
- [x] T021 [US3] Update `drawStatusBar()` in `src/renderer.cpp` to render `providerName` in the status bar area (left-aligned or after refresh time string, using `FONT_7pt8b` for consistency with existing status bar text)
- [x] T022 [US3] Update `src/main.cpp` to pass `weatherProvider.getName()` to `drawStatusBar()` call
...
- [x] T023 [US4] Update `drawCurrentSunrise()` and `drawCurrentSunset()` in `src/renderer.cpp` to check `isSentinelTimestamp(current.sunrise)` / `isSentinelTimestamp(current.sunset)` and render `"--"` if true
- [x] T024 [P] [US4] Update `drawCurrentWind()` in `src/renderer.cpp` to check `isSentinelFloat(current.wind_speed)` and render `"--"` if true
- [x] T025 [P] [US4] Update `drawCurrentHumidity()` in `src/renderer.cpp` to check `isSentinelInt(current.humidity)` and render `"--"` if true
- [x] T026 [P] [US4] Update `drawCurrentUVI()` in `src/renderer.cpp` to check `isSentinelFloat(current.uvi)` and render `"--"` if true
- [x] T027 [P] [US4] Update `drawCurrentPressure()` in `src/renderer.cpp` to check `isSentinelInt(current.pressure)` and render `"--"` if true
- [x] T028 [P] [US4] Update `drawCurrentVisibility()` in `src/renderer.cpp` to check `isSentinelInt(current.visibility)` and render `"--"` if true
- [x] T029 [P] [US4] Update `drawCurrentDewpoint()` in `src/renderer.cpp` to check `isSentinelFloat(current.dew_point)` and render `"--"` if true
- [x] T030 [US4] Update `drawCurrentAirQuality()` in `src/renderer.cpp` to check if air quality data is unavailable (`num_aq_hours == 0` or sentinel AQI values) and render `"--"` if true
- [x] T031 [US4] Update `drawForecast()` in `src/renderer.cpp` to check sentinel values on `daily[i].temp.max`, `daily[i].temp.min`, and precipitation before rendering; display `"--"` for any unavailable forecast metric
- [x] T032 [US4] Update `drawOutlookGraph()` in `src/renderer.cpp` to skip sentinel hourly data points in the temperature line and precipitation bars gracefully (do not plot sentinel values)
- [x] T033 [US4] ~~Compile and verify clean build with `-Wall`~~ SKIPPED per user request — manual compile at end
...
- [x] T034 Remove or mark `include/api_response.h` and `src/api_response.cpp` as deprecated — verify no remaining `#include "api_response.h"` references exist outside of `src/providers/owm_provider.cpp` (which may still use it internally or have absorbed its logic)
- [x] T035 ~~Final compile with `pio run`~~ SKIPPED per user request — manual compile at end — verify zero warnings with `-Wall`
- [x] T036 Verify `sizeof(weather_data_t)` does not exceed available SRAM budget by logging it at startup under `DEBUG_LEVEL >= 1` in `src/main.cpp`
- [x] T037 Document pre-refactor wake cycle active time — no code change needed, existing `beginDeepSleep()` prints this (from serial output "Awake for X.XXXs") and compare against post-refactor time to verify SC-003 (<=5% increase). The existing `beginDeepSleep()` in `src/main.cpp` already prints this value — no code change needed, just record and compare.
